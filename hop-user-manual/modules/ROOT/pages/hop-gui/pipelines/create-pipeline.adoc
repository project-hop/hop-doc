...
When you are finished with your pipeline, save it. This can be done via the File menu, the icons or using CTLR s or Command s. For new pipelines a file browser is displayed to navigate towards the location you want the store the file.
...
The transform object can be configured by right clicking on the object. The menu displayed below will be shown based on your transform object.
...
== Pipeline properties
Pipeline properties are a collection of properties that describe the piple and configure its behavior. 

The properties can be configured by double clicking on the pipeline canvas or by selecting the 'Edit pipeline' option in the metastore browser.
...
The monitoring tab allows you to specif the monitoring of the pipeline.
