...
|Hop CLI    |Hop CLI is a command line interface (CLI) for headless execution of Workflows and Pipelines |TBD
|=======
...
== Metastore
The Hop Metastore is the central storage repository for metadata (database connections, Workflows, Pipelines, ...), configuration, settings etc.
